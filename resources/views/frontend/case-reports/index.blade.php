<x-frontend-layout>
    <div class="mx-auto max-w-7xl px-2 sm:px-6 lg:px-8 min-h-screen">

        <div class="flex flex-col md:flex-row py-5 my-5 text-left max-w gap-6">
            <div class="md:w-1/4 px-6 pb-6 bg-blue-50 rounded min-h-[80vh]">
                <h1 class="text-2xl my-8 tracking-tight font-bold font-serif text-gray-dark text-left text-blue-800">
                    Case Reports
                </h1>
                <p class="my-12 text-lg leading-8 text-gray-600">
                    Recently published cases.
                </p>
                <p class="text-lg">
                    {{-- Stats: --}}
                </p>
            </div>

            <div class="md:w-3/4 px-6 pb-6">
                @foreach($cases as $case)

                    <a href="{{url("/case-reports/$case->id")}}">

                        <article class="mb-12 relative isolate flex flex-col gap-x-8 lg:flex-row hover:translate-x-1  transition-all cursor-pointer">
                            <div class="relative aspect-[16/9] sm:aspect-[2/1] lg:aspect-square lg:w-48 lg:shrink-0">

                                    <?php
                                    $imageURL = "data:image/jpeg;base64,";
                                    if ($case->getMedia('case-report-images') != null && count($case->getMedia('case-report-images'))) {
                                        $imageURL = $case->getFirstMediaUrl('case-report-images');

                                    }
                                    ?>

                                <img src="{{$imageURL}}"
                                     alt="" class="absolute inset-0 h-full w-full rounded-full bg-gray-50 object-cover" />
                                <div class="absolute inset-0 rounded"></div>
                            </div>
                            <div>
                                <div class="flex items-center text-xs mt-1">
                                    <time datetime="{{ $case->created_at }}" class="text-gray-700">
                                        <span class="absolute inset-0"></span>
                                        {{ $case->created_at->format('d M, Y')}}
                                    </time>
                                    @if(isset($case->ecosystem))
                                        <a href="#"
                                           class="relative z-10 rounded-full bg-gray-50 px-3 py-1.5 font-medium text-gray-700">
                                            {{$case->ecosystem->name}}
                                        </a>
                                    @endif
                                </div>
                                <a href="{{url("/case-reports/$case->id")}}">
                                <div class="group relative max-w-xl">
                                    <h3 class="mt-1 text-lg font-semibold leading-6">
                                            <span class="absolute inset-0"></span>
                                            {{ $case->title}}
                                    </h3>
                                    <p class="mt-5 text-sm leading-6 group-hover:text-gray-900
                                    truncate-overflow-3-lines">
                                        {{ Str::limit($case->statement, 200, '...') }}
                                    </p>
                                </div>
                                </a>
                            </div>
                        </article>
                    </a>

                @endforeach
                <div class="mt-8">
                    {{$cases->links()}}
                </div>
            </div>
        </div>
    </div>


</x-frontend-layout>
