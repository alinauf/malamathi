...
>
    ...
    <header>
        <h2 class="text-lg font-medium text-gray-900">
            {{ __('Edit Island Category') }}
        </h2>

        <p class="mt-1 text-sm text-gray-600">
            {{ __("Update the details of the island category") }}
        </p>
    </header>

    <form action="{{url("island-category/$islandCategory->id")}}" method="POST">
        @method('PUT')
        @csrf
        ...
        <x-confirm-create-modal title="Are you sure"
                                subtitle="Please confirm if you would like to update the island category"/>
    </form>
</div>
